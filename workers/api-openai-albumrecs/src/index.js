export default {
    async fetch(request, env) {
        try {
            ...
            const prompt = `I enjoyed the album "${albumTitle}" by "${artistName}". What are 2-3 other albums I should listen to if I enjoy that genre and style? Avoid albums that are very popular and mainstream, instead recommending what could be considered "hidden gems". If you don't have verifiable information about the album being asked about, inform the user that the album name is not valid. Use Markdown for formatting. Link each album name to the URL "https://listentomore.com/album/<artist name>_<album name>" inline, replacing spaces with - and special characters with their encoded values in the album and artist names.`;
