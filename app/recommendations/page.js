      ...
      lovedTracks.forEach(async track => {
        ...
        try {
          ...
          const spotifyResponse = await fetch(
            `https://api-spotify-search.rian-db8.workers.dev/?q=${encodeURIComponent(`track:${track.title} artist:${track.artist}`)}&type=track`
          );
          ...
          <div key={index} className="track_item track_item_responsive">
            ...
            <div className="no-wrap-text">
              <p>
                ...
                <Link
                  href={`/artist/${encodeURIComponent(track.artist.replace(/ /g, '-').toLowerCase())}`}
                >
                  {track.artist}
                </Link>
